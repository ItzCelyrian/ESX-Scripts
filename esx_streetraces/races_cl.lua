...
local RACE_STATE_RECORDING = 7
local RACE_CHECKPOINT_TYPE = 47
local RACE_CHECKPOINT_FINISH_TYPE = 4
                ...
                if raceStatus.checkpoint == 0 then
                    ...
                    if GetDistanceBetweenCoords(position.x, position.y, position.z, checkpoint.coords.x, checkpoint.coords.y, 0, false) < config_cl.checkpointProximity then
                        ...
                        if raceStatus.checkpoint == #(race.checkpoints) then
                            ...
                            if config_cl.checkpointRadius > 0 then
                                local checkpointType = raceStatus.checkpoint < #race.checkpoints and RACE_CHECKPOINT_TYPE or RACE_CHECKPOINT_FINISH_TYPE
                                nextCheckpoint.checkpoint = CreateCheckpoint(checkpointType, nextCheckpoint.coords.x,  nextCheckpoint.coords.y, nextCheckpoint.coords.z, 0, 0, 0, config_cl.checkpointRadius, 116, 70, 37, 127, 0)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0 , 26) then
			RageUI.Visible(RMenu:Get('racing', 'main'), not RageUI.Visible(RMenu:Get('racing', 'main')))
		end
	end
end)